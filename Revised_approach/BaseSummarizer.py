...
"""
A basic summarizer algorithm used for demo purpose is mentioned as follows.
...
class BaseSummarizer:
	...
	def sentence_reorder( self, output_sentences, input ):
		output_sentences.sort(lambda s1:input.find(s1))
		return output_sentences
	
	def get_summary(self, input, num_sentences ):
		...
		most_frequent_words = [pair for pair in word_frequencies.keys()]
		...
		return self.sentence_reorder(output_sentences, input)
	
	def summarize(self, input, num_sentences):
		return " ".join(self.get_summary(input, num_sentences))